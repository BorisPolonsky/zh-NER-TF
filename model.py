...
from tensorflow.keras.layers import GRUCell, LSTMCell, Dropout, RNN
...
class Model:
    ...
    def lookup_layer_op(self):
        ...
        with tf.compat.v1.variable_scope("words", reuse=False):
            _word_embeddings = tf.compat.v1.get_variable(initializer=self.embeddings,
                                                         dtype=tf.float32,
        ...
        self.word_embeddings = tf.nn.dropout(word_embeddings, rate=self.dropout_pl)
...
class BiLSTM_CRF(Model):
    ...
    def biLSTM_layer_op(self):
        ...
        with tf.compat.v1.variable_scope("bi-lstm"):
            ...
            #     dtype=tf.float32)
            # Only one cell is needed for this function in tensorflow 2.0. It creates two new copies of cell with
            # the same configuration except for 'go_backwards'.
            # In case you want to specify a different kind of backward cell, you may try
            # hacking into RNN.backward_layer: (Not tested yet)
            # e.g.
            # fw_layer = RNN(LSTMCell(30))
            # config = RNN(LSTMCell(90)).get_config()
            # config['go_backwards'] = True
            # bw_layer = RNN(LSTMCell.from_config(config))
            # bi_rnn = Bidirectional(RNN(fw_layer))
            # rnn.backward_layer = bw_layer
            output = tf.keras.layers.Bidirectional(RNN(cell_fw, return_sequences=True), merge_mode="concat")(inputs=self.word_embeddings)
            output = tf.nn.dropout(output, rate=self.dropout_pl)
...
class BiDirectionalStackedLSTM_CRF(BiLSTM_CRF):
    ...
    def biLSTM_layer_op(self):
        ...
        with tf.compat.v1.variable_scope("bi-stacked-lstm"):
            fw_layer = RNN(inner_cells(), return_sequences=True)
            # cell_fw = tf.compat.v1.nn.rnn_cell.MultiRNNCell(inner_cells())
            # cell_bw = tf.compat.v1.nn.rnn_cell.MultiRNNCell(inner_cells())
            # (output_fw_seq, output_bw_seq), _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(
            #     cell_fw=cell_fw,
            #     cell_bw=cell_bw,
            #     inputs=self.word_embeddings,
            ...
            output = tf.keras.layers.Bidirectional(fw_layer, merge_mode="concat")
            output = tf.nn.dropout(output, rate=self.dropout_pl)
...
class VariationalBiRNN_CRF(BiLSTM_CRF):
    ...
    def __init__(self, args, embeddings, tag2label, vocab, paths, config):
        raise NotImplementedError("Not migrated to tensorflow 2.0 yet.")
        super().__init__(args, embeddings, tag2label, vocab, paths, config)
...
#
#         with tf.compat.v1.variable_scope("variational-bi-rnn"):
...
#
#         with tf.compat.v1.variable_scope("projection-layer"):
#             W = tf.compat.v1.get_variable(name="W",
#                                           shape=[2 * self.hidden_dim, self.num_tags],
#                                           initializer=tf.contrib.layers.xavier_initializer(),
#                                           dtype=tf.float32)
...
class IDCNN_CRF(Model):
    ...
    def idcnn_layer_op(self):
        ...
        with tf.compat.v1.variable_scope("idcnn"):
            with tf.compat.v1.variable_scope("input_proj"):
                input_filter = tf.compat.v1.get_variable("input_mapping_filter",
                                                         shape=[1, self.filter_width, self.embedding_dim,
                                                                self.num_filter])
                layer_input = tf.nn.convolution(input=layer_input, filter=input_filter,
            ...
            with tf.compat.v1.variable_scope("conv"):
                ...
                for repeat_i in range(self.n_repeat):
                    for layer_j, dilation in enumerate(self.dilation_rate):
                        with tf.compat.v1.variable_scope("dilated-conv-{}".format(layer_j + 1),
                                                         reuse=tf.compat.v1.AUTO_REUSE):
                            filter_w = tf.compat.v1.get_variable("filter_w",
                                                                 shape=[1, self.filter_width, self.num_filter,
                                                                        self.num_filter],
                                                                 initializer=tf.contrib.layers.xavier_initializer())
                            filter_b = tf.compat.v1.get_variable("filter_b", shape=[self.num_filter])
                ...
                final_output = tf.nn.dropout(final_output, rate=self.dropout_pl)
            with tf.compat.v1.variable_scope("output_proj"):
                conv_out_shape = tf.shape(input=final_output)
                final_output = tf.reshape(final_output, shape=[-1, conv_output_dim])
                proj_w = tf.compat.v1.get_variable(name="w", shape=[conv_output_dim, self.num_tags])
                proj_b = tf.compat.v1.get_variable(name="b", shape=[self.num_tags])
                final_output = tf.compat.v1.nn.xw_plus_b(final_output, proj_w, proj_b)
